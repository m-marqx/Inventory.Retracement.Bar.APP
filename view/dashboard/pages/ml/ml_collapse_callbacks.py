...
class MLCollapseCallbacks:
    ...
    @callback(
        Output("feat_params_collapse", "is_open"),
        Output("feat_params_icon", "className"),
        Input("feat_params_button", "n_clicks"),
        State("feat_params_collapse", "is_open"),
    )
    def toggle_feat_params_collapse(n_clicks, is_open):
        if not n_clicks:
            raise dash.exceptions.PreventUpdate
