...
def corr_plot_interactive(
    ...
    **kwargs : optional
        Additional elements to control the visualization of the plot. These additional
        arguments will be passed to the `go.Heatmap` function in Plotly.

        Specific kwargs used in this function:

        * colorscale: str or list, optional
            The colorscale to be used for the heatmap. It controls the mapping of data
            values to colors in the heatmap.

        * zmax: float, optional
            The maximum value of the color scale. It limits the upper range of the
            colorbar displayed on the heatmap.

        * zmin: float, optional
            The minimum value of the color scale. It limits the lower range of the
            colorbar displayed on the heatmap.

        * text: pd.DataFrame, optional
            A DataFrame containing text to display on the heatmap. This text will be
            shown on the heatmap cells corresponding to the correlation values.

        * texttemplate: str, optional
            A text template string to format the text display on the heatmap. This allows
            you to customize how the text appears, including the display of the correlation
            values.

        * textfont: dict, optional
            A dictionary specifying the font properties for the text on the heatmap. You
            can customize the font size, color, family, etc., for the text annotations.

        * x: list, optional
            The list of column names for the x-axis of the heatmap. It allows you to
            customize the labels displayed on the x-axis.

        * y: list, optional
            The list of row names for the y-axis of the heatmap. It allows you to
            customize the labels displayed on the y-axis.

        * z: pd.DataFrame, optional
            The 2D array representing the correlation matrix to be visualized. This is the
            core data for generating the heatmap, containing the correlation values.

        * Many more kwargs are available, e.g., "hovertemplate" to control the legend
            hover template, or options to adjust the borderwidth and opacity of the heatmap.
            For a comprehensive list of available kwargs, please refer to the Plotly
            Heatmap documentation.

        Kwargs can be supplied through a dictionary of key-value pairs (see above) and
        can be found in Plotly Heatmap documentation.
